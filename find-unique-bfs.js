// Connected BFS Region Generator & Unique Solution Verifier (count === 1)
...
function findUniqueBFS(size, sol) {
  const frogPositions = sol.map((c, r) => ({ r, c }));
  const dirs = [{ dr: -1, dc: 0 }, { dr: 1, dc: 0 }, { dr: 0, dc: -1 }, { dr: 0, dc: 1 }];

  for (let trial = 1; trial <= 50000; trial++) {
    ...
    while (unassigned > 0) {
      ...
      if (!grew && unassigned > 0) {
        // Fallback for remaining unassigned cells
