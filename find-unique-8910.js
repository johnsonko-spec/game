// Dedicated fast generator for N=8, 9, 10 strictly unique grids
...
function findAnchorForSize(size) {
  const sols = {
    8: [0, 3, 6, 1, 4, 7, 2, 5],
    9: [0, 3, 6, 1, 4, 7, 2, 5, 8],
  ...
  for (let trial = 1; trial <= 150000; trial++) {
    const grid = Array.from({ length: size }, () => new Array(size).fill(-1));
    const queues = [];

    // Force Region 0 to be a single-cell anchor
    ...
    while (unassigned > 0) {
      ...
      for (const regId of regOrder) {
        if (regId === 0 && queues[0].length >= 1) continue; // Single cell anchor!
        if (regId === 1 && queues[1].length >= 2 && Math.random() < 0.7) continue; // Small anchor!
...
[8, 9, 10].forEach(s => findAnchorForSize(s));
